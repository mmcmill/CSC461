...
var uvBuffers = []; //contains texture coordinate lists by set, in doubles
var triSetSizes = []; // this contains the size of each triangle set
var triangleBuffers = []; // lists of indices into vertexBuffers by set, in triples
var viewDelta = 0; // how much to displace view with each key press
var textureBuffer = []; // contains 2D texture for each set
var lightMapBuffer = [];
...
function setupWebGL() {
     ...
     try {
       if (gl == null) {
         ...
         gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true); // flip y coords of loaded textures
       }
     } // end try
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                //set up UV array
                inputTriangles[whichSet].glUV = [];
                
                var numUV = inputTriangles[whichSet].uvs.length;
                
                for(var whichSetUV = 0; whichSetUV<numUV; whichSetUV++){
					uvToAdd = inputTriangles[whichSet].uvs[whichSetUV]; 
					inputTriangles[whichSet].glUV.push(1-uvToAdd[0], uvToAdd[1]);
				}
                ...
                if(inputTriangles[whichSet].material.texture == false){
					textureBuffer[whichSet] = loadTexture(false);
				} else {
					textureBuffer[whichSet] = loadTexture(TEXTURES_URL_START + 	inputTriangles[whichSet].material.texture); // load texture into buffer
				}
                lightMapBuffer[whichSet] = loadTexture(TEXTURES_URL_START + "lightmap.gif"); // load light map
...
function loadTexture(url){
	...
	//put a pixel in the texture in case the image 
	//fails to load in by the time we use it
	if(url==false){
	...
	image.onload = function (){
		...
		//since we are using webg1, we must check if the image is a power of two
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        attribute vec2 aTextureCoord; // texture UV coord
    ...
    var fShaderCode = `
        ...
        uniform float uAlpha; // the alpha
        uniform bool Blinn_Phong;  // Blinn_Phong shading toggle
        uniform bool Light_Map; // lightmap toggle
        uniform sampler2D uSampler; //texture
        uniform sampler2D uLightMap; //lightmap
        ...
        void main(void) {
			if(Light_Map) {
				gl_FragColor = texture2D(uSampler, vTextureCoord) * texture2D(uLightMap, vTextureCoord);
			} else {
				gl_FragColor = texture2D(uSampler, vTextureCoord);
			}
			
			vec3 colorOut = vec3(1,1,1);
			if(Blinn_Phong) {
				// ambient term
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vTexCoordAttribLoc = gl.getAttribLocation(shaderProgram, "aTextureCoord");
                ...
                toggleLightMapULoc = gl.getUniformLocation(shaderProgram, "Light_Map");
                textureULoc = gl.getUniformLocation(shaderProgram, "uSampler");
                lightMapULoc = gl.getUniformLocation(shaderProgram, "uLightMap");
...
function renderModels() {
	...
	function drawSets(isOpaque){
		...
		for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
			...
			if(currSet.material.alpha != 1.0 && isOpaque) continue;
			if(currSet.material.alpha == 1.0 && !isOpaque) continue;
			...
			gl.uniform1f(alphaULoc, currSet.material.alpha); // pass in the alpha val
			gl.uniform1i(Blinn_PhongULoc, Blinn_Phong);
			gl.uniform1i(toggleLightMapULoc, Light_Map);
			
			 //load texture into texture unit 0
			gl.activeTexture(gl.TEXTURE0);
			gl.bindTexture(gl.TEXTURE_2D, textureBuffer[whichTriSet]);
			gl.uniform1i(textureULoc, 0);
			
			gl.activeTexture(gl.TEXTURE1);
			gl.bindTexture(gl.TEXTURE_2D, lightMapBuffer[whichTriSet]);
			gl.uniform1i(lightMapULoc, 1);
